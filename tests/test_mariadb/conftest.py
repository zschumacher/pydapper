...
def mariadb_setup(database_name, setup_sql_dir, server):
    import mariadb

    conn = mariadb.connector.connect(host=server, port=3307, user="pydapper", password="pydapper", autocommit=True)
    cursor = conn.cursor()
