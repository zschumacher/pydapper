...
`params` accepts one parameter record: a mapping, mapping subclass, mutable mapping, dataclass, `SimpleNamespace`, or
custom object that exposes missing attributes through `__getattr__`. Top-level list params are only for `execute` and
`execute_async`; read and scalar methods raise `InvalidParameterShapeException` for top-level lists before opening a
cursor.
